...
* **The digital vault backup:** You can't write Passkeys on paper. Periodically export your Password Manager vault to a USB drive (e.g., **.cxp**, **.1pux**, or **.json**). To keep this file safe from thieves, keep the USB drive otherwise disconnected and store the file inside an encrypted container using **VeraCrypt** or **Cryptomator**.
...
1.  **Vault access:** Since the Password Manager keeps a **local encrypted copy** on your device, you can still access your passkeys even if the servers go down or you are offline. **Verify this now:** Turn on Airplane Mode (or disconnect your PC/Mac) and ensure you can still open your vault and see your entries.
2.  **Email recovery:** Most sites still let you use "Forgot Password" or email links as a fallback, though they may add a "security delay" to protect you.
